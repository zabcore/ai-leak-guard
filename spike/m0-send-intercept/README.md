...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------ | ------- | ----------------------------------------------------------------------------------------------------------------- |
| 0    | Panel → **2. Find composer + send button**                                                                                           | Q3      | `DISCOVERY` — composer + send button resolved, `composerInShadow`, `reactRoot`                                    |
| 1    | Type `abc`, press **Enter** (sends `abc` — fine)                                                                                     | Q2, Q3  | `KEYDOWN` window-capture (isTrusted:true) → `KEYDOWN-BUBBLE` with `defaultPrevented:true` = site ran AFTER us     |
...
logging method + pathname only, never bodies).
